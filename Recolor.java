...
public class Recolor extends JFrame implements MouseListener, MouseMotionListener
{
  ...
  /*
  public Color[][] Blur(Color[][] original){
	  Locator(original);
	    ImageUtils image=new ImageUtils();
	    int red1=original[x1][y1].getRed();
	    int blue1=original[x1][y1].getBlue();
	    int green1=original[x1][y1].getGreen();
	    System.out.println(red1+","+blue1+","+green1);
	    for (int i=0; i<original.length; i++) {
	    	for(int j=0; j<original[0].length; j++) {
	    		int currentaverage=(original[i][j].getBlue()+original[i][j].getGreen()+original[i][j].getRed())/3;
	    		boolean avggreat=(((red+green+blue)/3)>=currentaverage-5);
	    		boolean avgless=(((red+green+blue)/3)<=currentaverage+5);
	    		if(avggreat&&avgless) {
	    				original[i][j]=Color.black;
	    				}else{
	    					original[i][j]=Color.white;}}
	    				}
	    x1=0;
	    y1=0;
	    return original;  
